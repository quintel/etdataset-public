...
for country in countries:
    ...
    temperature_file_path = data_folder / "input" / "temperature.csv"
    irradiation_file_path = data_folder / "input" / "solar.csv"
    output_file_path = data_folder / "output"
    Path.mkdir(output_file_path, exist_ok=True)
    ...
    for index, house_type in enumerate(house_types):
        # i=i+1
        for insulation_type in insulation_types:
            ...
            np.savetxt(output_file_path / f"insulation_{house_types_names[index]}_{insulation_type}.csv", hourly_data, fmt='%.10e', delimiter=',')
