...
def smooth_curve_with_moving_average(no_of_averages, curve):
	''' Extracted from main '''
	cumsum, moving_aves = [0], []

	for i, x in enumerate(curve, 1):
		cumsum.append(cumsum[i-1] + x)
		if i >= no_of_averages:
			moving_ave = (cumsum[i] - cumsum[i - no_of_averages]) / no_of_averages
			moving_aves.append(moving_ave)
		else:
			moving_aves.append(curve[i])
...
def main(year):
	...
	for turbine_type in types:
		...
		for weather_station in weather_stations[turbine_type]:
			...
			output_power_array_smooth = smooth_curve_with_moving_average(
				number_of_values_moving_average, output_power_array
			)
