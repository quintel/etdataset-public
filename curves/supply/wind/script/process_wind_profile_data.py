...
def perform_checks(data, country, year):
    ...
    threshold = 0.02
    missing = float(data['{}_wind_profile'.format(str.upper(country))].isnull().sum()) / 8760.0
    print('\nData coverage: {}%'.format((1.0 - missing) * 100))
    if missing >= threshold:
        print('\nWARNING: CURVE IS NOT GENERATED!\n(> 2% missing data points in {})'.format('{}_wind_profile'.format(str.upper(country))))
        return data, False
...
def fill_gaps(data, country):
    column = '{}_wind_profile'.format(str.upper(country))
