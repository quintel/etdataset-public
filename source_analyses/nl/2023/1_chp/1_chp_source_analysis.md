...
**Note:</br>
A big change since the 2019 dataset is that we mostly use Eurostat as source material instead of IEA. The content still refers to IEA source material a lot and those references may therefore no longer apply.**
...
**Question:**</br>
1. Is there an alternative Eurostat source for the CBS source cited?</br>
**Answer:**</br>
Probably not. For now, we use the CBS data. If we have time we will try and see if we can use Eurostat data to improve things.
...
**Question:**</br>
2. Does Eurostat also use energy statistics or does it use machine statistics?
</br>
**Answer:**</br>
Eurostat does both. The energy balance is machine statistics, meaning it lists all electricity production by CHPs, for example, not just electricity production in CHP mode. This was answered by comparing the Energy Balance data in the CHP_analysis and the [Eurostat source](./nrg_chp_cpfe__custom_16870923_spreadsheet.xlsx)


**Answer Eurostat (Marek Sturc):** Eurostat treats CHPs the same way as the *national statistical agency* that chooses how to treat CHPs. Eurostat will provide a more detailed table outlining exactly what power and heat is produced per unit. In this, they will also specify </br> 
...
## Changes
The following changes were made for the 2023 dataset w.r.t. the 2019 dataset:

- According to Eurostat the fuel input in CHPs and the heat production by CHPs are respectivly 412,357 TJ and 143,939 TJ. This matches the CBS source very closely. See [1_chp_source_analysis](1_chp_source_analysis.xlsx).
- The `Percentage of el. prod. with waste carriers` that is removed from Services sector value was changed to 42.07% to optimize `Results by fuel`.
...
- **Agriculture:** According to the Eurostat there is little sold heat production in the agriculture sector. Furthermore, almost all of the CHPs are the 'Gasmotor' type (CBS), so the percentage of CHP electricity from wood pellets is close to 0%. The [1\_chp\_source\_analysis](1_chp_source_analysis.xlsx) calculates its share.
- **Households:** According to the Eurostat Autoproducer table there are no CHPs in households.
- **Services:** Eurostat reports waste incineration as part of the services sector, but in the ETM this is Main Activity. We therefore subtract Waste CHP production from the services sector. Less CHP electricity production remains after that. The `chp_analysis` dashboard requires you to assume what percentage of electricity production with waste carriers should be subtracted from the services sector. The subtraction itself occurs on the `fuel_aggregation` sheet in the `chp_analysis`. 
- **Services:** The unsold heat percentage for gas CHPs is set to 0% as an assumption. All unsold heat is produced by biomass (wood pellet) CHPs. 
- **Services:** I had to correct and error in the CHP analysis in commit `517454d6`. This led to negative final demand for `Non-specified primary biofuels and waste` because it used the same correction factor as for `Primary solid biofuels` and subtracted the number in `Results by fuel'!G2` and `Results by fuel'!G31` both in column AU and in column AZ for the Residential and Commercial and public services sectors. The fix seems to have corrected this mistake.
...
- **Energy Industry:** The share of gas turbine CHPs (53.8%), gas engine CHPs (0%) and gas combined cycle CHPs (46.2%) are calculated in the [1\_chp\_source\_analysis](1_chp_source_analysis.xlsx).
...
- **Industry:** The share of gas turbine CHPs (23.5%), gas engine CHPs (1.8%) and gas combined cycle CHPs (74.7%) are calculated in the [1\_chp\_source\_analysis.xlsx](1_chp_source_analysis.xlsx).
- **Main activity:** The 50/50 co-firing shares assumption results in too much sold heat from coal. This is hard to prevent. It leads to a similar amount of deficit in sold heat for biomass and too little fuel input of biomass for fuel input for electricity and sold heat. This cannot be fixed with efficiencies, as this increases the surplus of coal input. 

- **Full load hours:**
  - Agriculture CHPs optimized to match the [1\_chp\_source\_analysis.xlsx](1_chp_source_analysis.xlsx). 
  - Heat network CHPs optimized to match the [1\_chp\_source\_analysis.xlsx](1_chp_source_analysis.xlsx). There is an initial discrepancy between electricity production from the Services sector as reported by CBS and reported  by Eurostat. The [1\_chp\_source\_analysis.xlsx](1_chp_source_analysis.xlsx) shows that according to CBS there was 2,030 TJ electricity produced by CHPs in the Services sector. The `Fuel mixes` sheet in the `chp_analysis` shows that according to Eurostat, w/o waste, there was 9,595 TJ electricity produced by CHPs in the Services sector. This is likely caused by a difference in defition between central/decentral from CBS and main activity/autoproducer in Eurostat. The effect is that, even with 8500 FLHs for the heat network CHPs, the resulting installed capacities for the Gas CHP and Biogas CHP are much higher than what is stated by CBS.
  For the wood pellets CHPs, the installed capacitiy is ~58 MWe too high. This is the result of moving wood pellets out of industry (see step 2 above).
   - Industry & Energy industry CHPs optimized to match the [1\_chp\_source\_analysis.xlsx](1_chp_source_analysis.xlsx). Since the source analysis does not contain the coal gas CHP yet, this was assumed to be included in the total `Gas CC CHP` installed capacity for this sector according to CBS. The installed heat capacities cannnot be reproduced without tweaking efficiencies, by these cannot be tweaked too far, as this resuls in errors in the `Results by fuel` tab. So there is a large thermal CHP deficit in this sector of about 6.8 GWth! This I cannot fix. It is probably caused by misreporting sold and unsold heat, but I am not sure.
   - Main acivity FLH for `coal` and `co-firing` cannot meet the installed capacity in the [1\_chp\_source\_analysis.xlsx](1_chp_source_analysis.xlsx) without having super low FLH. The capacity in the source analysis is suspect, so I tried instead to set FLH to represent the 600 MWe of the Amer 9 power plant. This is listed as the only coal-fired (and biomass co-firing) CHP in NL in 2023. For this, the FLH have to be set to higher than 8760, which indicates that quite some of the power generated and assigned to co-foring CHPs is probably generated by pure biomass CHPs. In the end I just set the FLH to 4500 by assumption. This results in 1469 MWe of co-firing and coal CHP and includes an unkown capacity of biomass plants. </br>
Also, it is likely that quite a few MWe in the number in the source analysis are in fact gas-fired steam turbine CHPs , which are not available in the ETM. </br>

If this is true, we have a deficit of 5,313 - 4,877 = 436 MWe in gas-fired steam turbine CHP capacity **will have to becompensated in `2_chp_pp_analysis.xlsx`. Results by machine** to represent the entire NL power and CHP production park. **This deficit is passed onto the PPHP source analysis.**
...
| :-- | :----------- | -----------: |  ----------: | -----------: |
| Agriculture - Gas CHP | 43% | 47% | 37% | 48% |
| Agriculture - Biogas CHP | 43% | 47% | 44% | 31% |
| Agriculture - Wood pellets CHP | 29% | 82% | 10% | 82% |
| Heat network local - Gas CHP | 43%  | 47% | 37% | 48% | 
| Heat network local - Biogas CHP | 43%  | 47% | 44% | 31% | 
| Heat network local - Wood pellets CHP | 29%  | 82% | 14% | 30% | 
| Industry & EI - Gas turbine CHP | 38% | 42% | 38% | 44% | 
| Industry & EI - Gas engine CHP | 43% | 47% | 43% | 47% | 
| Industry & EI - Gas combined cycle CHP | 46% | 42% | 42% | 33% | 
| Industry % EI - Coal CHP | 40%  | 15% | 40% | 15% | 
| Industry % EI - Wood pellets CHP | 18%  | 47% | 18% | 47% | 
...
| CHP | E-eff default | H-eff default | E-eff NL2019 | H-eff NL2019 |
| :-- | :----------- | -----------: |  ----------: | -----------: |
| Gas combined cycle CHP | 46%  | 42% | 42% | 33% | 
| Coal CHP | 40%  | 15% | 44% | 5% | 
| Co-firing CHP | 37% | 15% | 42% | 10% | 
| Waste CHP | 27% | 15% | 20% | 23% | 
